...
class _HomePageState extends State<HomePage> {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text(
              "Failed to establish a connection with the database. Make sure you are connected to the internet and restart the app."),
          actions: [
            TextButton(
              onPressed: () {
  ...
  Future<void> _loadData(bool initialize) async {
    if (initialize) {
      ...
      previousWeekSummary =
          await DatabaseService.instance.fetchPreviousWeekSummary();
    }
    ...
    await Future.delayed(const Duration(
        milliseconds: 500)); //Wait for a while so the loading is visible
    setState(() {
  ...
  Widget _streakBlock() {
    Widget streakText;
    if (streak == null) {
      streakText = Text("Loading streak...");
    } else if (streak == -1) {
      streakText = Row(
      ...
      streakText = Row(mainAxisAlignment: MainAxisAlignment.center, children: [
        Text(style: TextStyle(fontSize: 30), "Streak: $streak"),
        Icon(
          Icons.water_drop,
          color: DatabaseService.instance.streakActive
            ...
            children: [
              ...
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Row(children: [
                  ...
                  Expanded(
                      child:
                          Text('An upward arrow means you are feeling better.'))
                ]),
                SizedBox(height: 10),
                Row(
                  children: [
                    ...
                    Expanded(
                        child: Text(
                            "A flat line means your mood stayed about the same."))
                  ],
                ),
                SizedBox(height: 10),
                Row(children: [
                  ...
                  Expanded(
                      child: Text(
                          'A downward arrow means your mood has decreased.'))
                ])
              ]),
            ],
          ),
  ...
  Widget _gradeBlock() {
    ...
    bool showTrend =
        previousWeekAverageMood != null && currentWeekAverageMood != null;
    int moodTrend = showTrend
        ? (currentWeekAverageMood -
                previousWeekAverageMood / previousWeekAverageMood * 100)
            .toInt()
        : 0;
    return CustomBlock(
        child: Column(children: [
      ...
      Padding(
        ...
        child: Image.asset(flowers[(currentWeekAverageMood ?? 2).round()],
    ...
    return Padding(
        ...
        child: Row(
            ...
            children: List.generate(columnCount, (columnIndex) {
              int activeCount = columns.contains(columnIndex)
                  ? (moodCount[columnIndex] / countSum).floor()
                  : 0;
              ...
              return Column(
                ...
                if (index == rowCount) {
                  return Padding(
                      ...
                      child: Icon(faces[columnIndex],
                          color: activeColors[columnIndex]));
                }
                ...
                return Padding(
                  padding:
                      EdgeInsets.only(bottom: index < rowCount - 1 ? 2.0 : 0),
                  child: Container(
                      width: 50,
                      height: 8,
  ...
  Widget _emotionsBlock() {
    ...
    if (!isSummaryLoading && !weekSummary.isEmpty) {
      ...
      fetchedEmotions = emotionsData.isNotEmpty && !isSummaryLoading
          ? Wrap(
              spacing: 8,
              children: emotionsData.map((e) {
                return LabeledIconChip(
                    label: e, iconCodePoint: weekSummary.icons![e]);
              }).toList(),
            )
          : Row(
              mainAxisAlignment: MainAxisAlignment.center,
